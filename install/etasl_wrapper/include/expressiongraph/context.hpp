...
#include <expressiongraph/velocityestimator.hpp>
#include <expressiongraph/expressiontree_traits.hpp>

//#include <iostream>

#define NO_ADD_OUTPUT
...
class VelocityEstimatorEval {
public:
    typedef boost::shared_ptr< VelocityEstimatorEval > Ptr;
    virtual void evaluate(double dt) = 0;
    virtual void initialize(double dt) = 0; 
    virtual const std::string& getName() = 0;
    virtual std::string toString() = 0;
    virtual ~VelocityEstimatorEval() {}
};

template <typename T>
class VelocityEstimatorEvalImpl : public VelocityEstimatorEval {
    typedef typename AutoDiffTrait<T>::DerivType DerivT;
    typename VelocityEstimator<T>::Ptr velestim;
    typename Expression<T>::Ptr expr;        // outp. expr: value
    typename VariableType< DerivT >::Ptr ic; // inp. channel: num. deriv.
    const std::string name;
public:

    VelocityEstimatorEvalImpl(
        const std::string& _name,
        typename VelocityEstimator<T>::Ptr&  _velestim,
        typename Expression<T>::Ptr& _expr,
        typename VariableType<DerivT >::Ptr& _ic
    ):name(_name), velestim(_velestim), expr(_expr), ic(_ic) {}

    virtual void initialize(double dt) {
        velestim->initialize(dt);
    }
    virtual void evaluate(double dt) {
        ic->setValue(  velestim->estimateVelocity(expr->value(),dt) );
    }
    virtual const std::string& getName() {
        return name;
    }
    virtual std::string toString() {
        return velestim->toString();
    }
    virtual ~VelocityEstimatorEvalImpl() {}
};
...
class Context : public boost::enable_shared_from_this<Context>
{
    ...
    public:
        ...
        //----------------------------------------------------------
        /** 
         * \name MEMBERS: velocity estimators 
         */
        /// @{  
        std::vector< VelocityEstimatorEval::Ptr > velestim_ptrs;    ///< list of pointers to velocity estimators
...
public:
        ...
        typename Expression<T>::Ptr createInputChannel(const std::string& name, const T& initial_value) {
            if (input_vars.find(group_name()+"."+name)!=input_vars.end()) {
                throw "duplicate name for input channel";
            }       
            typename VariableType<T>::Ptr tmp = Variable<T>(getScalarNdx("time"),1);
            input_vars[group_name() + "." + name] = tmp;
        ...
        void setOutputExpression(const std::string& name, typename Expression<T>::Ptr expr) {
            if (output_vars.find(group_name()+"."+name)!=output_vars.end()) {
                throw "duplicate name for output expression";
            }            
        ...
        //----------------------------------------------------------
        /** 
         * \name METHODS for specification of velocity estimators
         */
 
        /// @{  
            
        /**
         * Creates an InputChannel and OutputExpression with the same name and connects these to an Velocity Estimator.
         * @param name name of the velocity estimator
         * @param expr expression where we want to compute the total time derivative from.
         * @param velestim class encapsulating the velocity estimation algorithm (and the initial value),
         *    underlying type on which this class operates should correspond to the underlying type of expr.
         * @return an expression that evaluates to the estimated value for the derivative (all partial derivatives zero)
         */
        template <typename T>
        typename Expression< typename AutoDiffTrait<T>::DerivType >::Ptr 
        createVelocityEstimator( 
            const std::string& name, 
            typename Expression<T>::Ptr expr,
            typename VelocityEstimator<T>::Ptr velestim) {
                for (size_t i=0; i< velestim_ptrs.size();++i) {
                    if (velestim_ptrs[i]->getName()==name) {
                        throw "duplicate name for velocity estimator";
                    }
                }
                if (output_vars.find(group_name()+"."+name)!=output_vars.end()) {
                    throw "velocity estimator : name already exists for an output expression";
                }
                if (input_vars.find(group_name()+"."+name)!=input_vars.end()) {
                    throw "velocity estimator : name already exists for an input channel";
                }
                typedef typename AutoDiffTrait<T>::DerivType DerivT;
                setOutputExpression<T>(name, expr);
                typename Expression<DerivT>::Ptr retval = createInputChannel<DerivT>(name);
                typename VariableType<DerivT>::Ptr ic = getInputChannel<DerivT>(name);
                velestim_ptrs.push_back( boost::make_shared<VelocityEstimatorEvalImpl<T> >(name,velestim,expr,ic) );
                return retval;
            }

        /**
         * @brief evaluates all the velocity estimators
         * 
         * @param dt sample time
         */
        void evaluateVelocityEstimators(double dt) {
            for (size_t i=0;i<velestim_ptrs.size();++i) {
                velestim_ptrs[i]-> evaluate(dt);
            }
        }

        /**
         * @brief initializes all the velocity estimators
         * 
         * @param dt sample time
         */
        void initializeVelocityEstimators(double dt) {
          for (size_t i=0;i<velestim_ptrs.size();++i) {
                velestim_ptrs[i]-> initialize(dt);
            }            
        }

        /**
         * @brief print information on the velocity estimators
         * 
         */
        void printVelocityEstimators(std::ostream& os);
        ...
        virtual ~Context() {
            //std::cout << "~Context() is called" << std::endl;
        }
        /// @}
};
