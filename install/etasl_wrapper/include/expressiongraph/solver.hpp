...
protected:
        ...
        double              weight_factor;      ///< used with the FULL initialization procedure  
        bool                full_initialization; ///< true when full initialization was called. 
        /// @}
public:

    /** Common error codes for all solvers: **/
    enum ErrorCode {
        SUCCESS = 0,
        DISPLAY_ERRORS=200
    };
     ...
     * returns -1.0 if not implemented.
     */
    ...
    /**
     * Prepare for  the initalization using an alternative approach
     * (compared to prepareInitialization())
     * In this alternative approach:
     *    - robot variables are assumed constant
     *    - priority 0 and 1 constraints are implemented as soft constraints with a given heigh weight
     *    - priority 2 constraints are implemented as soft constraints with their assigned weight 
     *    - Box constraints (i.e. on a single variable at velocity level)  are still "hard", since these box constraints
     *      by themself cannot lead to infeasibility (as long lowerbound <= upperbound).
     *
     * \param [in] ctx Context to initialize  the solver for.
     * \param [in] weight_factor: factor to multiply priority 0 and 1  constraints with.
     *                (should be significantly larger than the heighest weight of priority 2)
     *
     * \return 0 if sucessful, returns:
     *    - -1 if an unknown priority level is used.
     *    - 1 if not implemented by the solver (falls back to the old prepareInitialization() )
     */
	virtual int prepareInitializationFull(Context::Ptr ctx, double weight_factor) {
        prepareInitialization(ctx);
