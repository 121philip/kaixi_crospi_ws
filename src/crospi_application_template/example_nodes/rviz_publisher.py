...
This CroSPI-side copy of RVizPublisher is not used by the current runtime path.
The active UDP sender lives in lerobot_trossen/important_code/inference/rviz_publisher.py.
Keep CroSPI as the UDP receiver/ROS bridge via vla_ros_bridge_node.py.

RViz 可视化发布器（UDP 发送端）。

架构：venv 中的 Python 3.12 无法直接 import rclpy（rclpy 编译目标为系统 Python 3.10）。
解决方案：
  - vla_ros_bridge_node.py 由 trossen_follower_visualization.launch.py 在已 source ROS2 环境中启动，
    负责接收 UDP 数据并发布 ROS2 话题
  - 本模块（rviz_publisher.py）在 Python 3.12 主进程中运行，通过 UDP socket 向 vla_ros_bridge_node.py 发送数据
...
发布的 ROS2 话题（由 vla_ros_bridge_node.py 发布）：
  /joint_states_VLA          → 6-DOF VLA 目标（供 eTaSL 跟踪）
  /actual/joint_states_VLA   → 蓝色实体机器人（来自真实机器人反馈 /joint_states）
  /predicted_ee_marker       → 绿色球（当前 EE 位置）+ 橙红色线（N步预测轨迹）

使用方式（由 run_inference_rtc.py 通过 --rviz 标志自动创建）：
  1. 先启动：ros2 launch crospi_application_template trossen_follower_visualization.launch.py
